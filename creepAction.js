...
function avoidEnemy(creep) {
    ...
    if (!isEnemyCombatant && creep.room.name === creep.memory.colony) {
        return ERR_NOT_FOUND
    }

    if (isEnemyCombatant) {
        for (const enemy of enemyCombatants) {
            if (creep.pos.getRangeTo(enemy.pos) < 5) {
                creep.fleeFrom(enemy, 20)
                return OK
            }
        }

        const isDefender = creep.room.getIsGuard()
        if (isDefender) {
    ...
    if (creep.memory.working) {
        if (!creep.memory.targetId) {
            if (creep.room.constructionSites.length) {
                creep.memory.targetId = creep.pos.findClosestByRange(creep.room.constructionSites).id
            } else {
                creep.memory.targetId = undefined
            }
        }

        const target = creep.memory.targetId ? Game.getObjectById(creep.memory.targetId) : undefined
...
function colonyMiner(creep) {
    ...
    if (container) {
        creep.setWorkingInfo(container.pos, 0)
    } else {
...
function colonyHauler(creep) {
    ...
    if (creep.memory.supplying) {
        ...
        if (constructionSites.length > 0 && creep.getActiveBodyparts(WORK) > 0) {
            if (creep.room.name !== creep.memory.colony || isEdgeCoord(creep.pos.x, creep.pos.y)) {
                creep.moveToRoom(creep.memory.colony)
                return
            }

            if (!creep.memory.targetId) {
                creep.memory.targetId = creep.pos.findClosestByRange(constructionSites).id
            }
    ...
    if (source.container && source.container.store[RESOURCE_ENERGY] >= Math.min(creep.store.getFreeCapacity(), 100)) {
        creep.getEnergyFrom(source.container.id)
        return
    }
...
function colonyCoreDefender(creep) {
    ...
    const hostileCreeps = creep.room.findHostileCreeps().filter(creep => creep.checkBodyParts(['attack', 'ranged_attack', 'heal']))
...
function guard(creep) {
    ...
    if (!creep.memory.harass && creep.ticksToLive < 500) {
...
module.exports = { miner, extractor, reserver, claimer, pioneer, colonyLaborer, colonyMiner, colonyHauler, colonyDefender, colonyCoreDefender, wallMaker, researcher, guard }