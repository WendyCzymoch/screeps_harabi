...
Overlord.manageDepositTasks = function () {
  ...
  for (const depositRequest of Object.values(tasks)) {
    ...
    if (depositRequest.completed === true) {
      data.recordLog(`DEPOSIT: ${roomInCharge.name} completed DEPOSIT mining at ${targetRoomName}. returned ${depositRequest.amountReturned || 0} ${depositRequest.depositType}`, targetRoomName)
      this.deleteTask(depositRequest)
      return
    }
