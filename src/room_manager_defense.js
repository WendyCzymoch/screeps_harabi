...
const RAMPART_HITS_TO_REPAIR_WITH_TOWERS = 2000;
...
Room.prototype.manageDefense = function () {
  ...
  } else if (this.memory.publicRampart) {
    for (const rampart of this.structures.rampart) {
      rampart.setPublic(false);
    }
    this.memory.publicRampart = false;
  ...
  if (invulnerables.length > 0 && status.state === 'normal' && this.isWalledUp && attackPowerTotal > 0) {
    ...
    (attackPowerTotal === 0 || this.controller.safeMode)
  ) {
    ...
    const repairingForNuke =
      this.memory.defenseNuke &&
      ['build', 'repair'].includes(this.memory.defenseNuke.state) &&
      this.energyLevel > config.energyLevel.REACT_TO_NUKES;
  ...
  if (
    invulnerables.length > 0 &&
    this.getMyCombatants().length === 0 &&
    !this.isWalledUp &&
    this.controller.level >= RCL_THRESHOLD_TO_SAFEMODE &&
  ...
  if (status.state === 'emergency' && this.isWalledUp) {
...
Room.prototype.manageTower = function (targets) {
  ...
  if (this.structures.damaged.length) {
    outer: for (const structure of this.structures.damaged) {
  ...
  if (
    weakestRampart.hits < RAMPART_HITS_TO_REPAIR_WITH_TOWERS &&
    (!this.storage || this.storage.store[RESOURCE_ENERGY] > 5000)
  ) {
    const weakRamparts = this.structures.rampart
      .filter((ramart) => ramart.hits < RAMPART_HITS_TO_REPAIR_WITH_TOWERS)
      .sort((a, b) => a.hits - b.hits);
    outer: for (const structure of weakRamparts) {
      const towers = this.structures.tower.filter((tower) => !tower.busy);
      if (towers.length === 0) {
        break outer;
      }
...
Room.prototype.manageEmergency = function () {
  ...
  for (const status of Object.values(rampartAnchorsStatus)) {
    ...
    const positionsToSit = rampartPositions.filter(
      (pos) => pos.getTaxiRangeTo(status.closestIntruder.pos) <= status.closestRange + 1
    );
    ...
    defenders.forEach((defender) => {
      ...
      const goals = positionsToSit.map((pos) => {
        return { pos: pos, range: 1 };
      });
      if (range > 1) {
        defender.moveMy(goals);
        return;
      }
    ...
    const goal = positionsToSit.map((pos) => {
      return { pos, range: 1 };
    });
    for (const pos of positionsToSit) {
...
Room.prototype.assignDefenders = function () {
  ...
  // check anchors
  outerA: for (const status of anchorStatusesSorted) {
    if (status.requiredDamageMax <= 0) {
      continue;
    }
  ...
  outerB: while (freeDefenders.length > 0 && anchorStatusesSorted.length > 0) {
...
Room.prototype.assignLaborers = function () {
  ...
  // check anchors
  outer: for (const status of anchorStatusesSorted) {
    if (status.totalAttackPower <= 0) {
      continue;
    }
    const packed = packCoord(status.pos.x, status.pos.y);
  ...
  outer: while (freeLaborers.length > 0 && anchorStatusesSorted.length > 0) {
    for (const status of anchorStatusesSorted) {
...
Room.prototype.getRampartAnchorsStatus = function () {
  ...
  const intruders = this.findHostileCreeps().filter((creep) =>
    creep.checkBodyParts(['work', 'attack', 'ranged_attack', 'heal'])
  );
  const result = {};
  const rampartAnchors = this.rampartAnchors;
  ...
  for (const rampartAnchor of rampartAnchors) {
    ...
    if (!status || !status.intruders || status.intruders.length === 0) {
      continue;
    }
    ...
    for (const intruder of status.intruders) {
      ...
      const requiredDamage =
        this.getRequiredDamageFor(intruder, { netDamage: netDamage, assumeFullPower: true }) -
        this.frontLineTowersDamageMin;
    ...
    this.visual.text(`👿🗡️${status.totalAttackPower}`, rampartAnchor.x, rampartAnchor.y - 0.5, {
      color: 'magenta',
      font: 0.5,
    });
...
Room.prototype.getTowersDamageFor = function (target) {
...
RoomPosition.prototype.getTowerDamageAt = function () {
  const towers = Game.rooms[this.roomName].structures.tower.filter((tower) => tower.store[RESOURCE_ENERGY] >= 10);
...
StructureTower.prototype.getAttackDamageTo = function (target) {
  //target은 roomPosition 혹은 roomPosition 가지는 Object
  ...
  const fallOffRatio =
    (TOWER_FALLOFF * (effectiveRange - TOWER_OPTIMAL_RANGE)) / (TOWER_FALLOFF_RANGE - TOWER_OPTIMAL_RANGE);
...
Object.defineProperties(Room.prototype, {
  ...
  frontLineTowersDamageMin: {
    get() {
      ...
      return (this._frontLineTowersDamageMin = this.heap._frontLineTowersDamageMin =
        this.getFrontLineTowersDamageMin());
    },
  },
...
Room.prototype.getDefensiveAssessment = function () {
  ...
  const killerCreeps = this.findHostileCreeps().filter((creep) =>
    creep.checkBodyParts(['attack', 'ranged_attack', 'work', 'heal'])
  );
  ...
  return (this._defensiveAssessment = this.heap._defensiveAssessment =
    { costs: costMatrix, frontLine, frontRampartPositions });
};
...
Creep.prototype.getRangedAttackPower = function () {
  ...
  const boostrequiredResources =
    this.room.isMy && this.room.boostQueue[this.name] ? this.room.boostQueue[this.name].requiredResources : undefined;
...
Room.prototype.getTotalHealPower = function (target) {
  //target은 hostile creep
  let result = 0;
