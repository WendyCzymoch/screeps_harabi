...
Room.prototype.operateLab = function (resource0, resource1) {
  ...
  if (this.labState === 'produce') {
    if (
      sourceLabs[0].store[resource0] >= LAB_REACTION_AMOUNT &&
      sourceLabs[1].store[resource1] >= LAB_REACTION_AMOUNT
    ) {
      for (const lab of reactionLabs) {
        if (lab.runReaction(sourceLabs[0], sourceLabs[1]) === -10) {
          // reaction lab에 이상한 거 들어있으면 terminal에 반납해라
          if (!researcher) {
  ...
  if (this.labState === 'prepare') {
    if (
      (sourceLabs[0].mineralType && sourceLabs[0].mineralType !== resource0) ||
      (sourceLabs[1].mineralType && sourceLabs[1].mineralType !== resource1)
    ) {
      this.memory.labState = 'transfer'
      return
    }
    if (sourceLabs[0].store[resource0] < AMOUNT_REQUIRED_TO_MAKE_BOOSTS) {
      if (
        researcher.isFree &&
        terminal.store[resource0] >= AMOUNT_REQUIRED_TO_MAKE_BOOSTS - sourceLabs[0].store[resource0]
      ) {
    ...
    if (sourceLabs[1].store[resource1] < AMOUNT_REQUIRED_TO_MAKE_BOOSTS) {
      if (
        researcher.isFree &&
        terminal.store[resource1] >= AMOUNT_REQUIRED_TO_MAKE_BOOSTS - sourceLabs[1].store[resource1]
      ) {
...
Object.defineProperties(Room.prototype, {
  labs: {
    get() {
      if (
        this.memory.labs &&
        this.memory.labs.sourceLab.length === 2 &&
        this.memory.labs.sourceLab.length + this.memory.labs.reactionLab.length === this.structures.lab.length
      ) {
...
Room.prototype.getLabTarget = function () {
  ...
  function getRatio(resourceType) {
    ...
    const currentAmount = Memory.stats && Memory.stats.resources ? Memory.stats.resources[resourceType] || 0 : 0
    return currentAmount / goalAmount
  }

  const targetCompounds = []

  const numMyRooms = Overlord.myRooms.length

  for (const resourceType of Object.keys(COMPOUND_GOAL)) {
    const ratio = getRatio(resourceType)
    if (ratio > numMyRooms) {
      continue
    }
    targetCompounds.push({ resourceType, ratio })
  }

  targetCompounds.sort((a, b) => a.ratio - b.ratio)

  const checked = {}
  //target 확인
  for (const targetCompound of targetCompounds) {
    const target = targetCompound.resourceType
...
Room.prototype.checkCompound = function (compound) {
  ...
  if (
    terminal.store[formula.resourceType0] < AMOUNT_REQUIRED_TO_MAKE_BOOSTS ||
    terminal.store[formula.resourceType1] < AMOUNT_REQUIRED_TO_MAKE_BOOSTS
  ) {
