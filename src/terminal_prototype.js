...
StructureTerminal.prototype.run = function () {
  ...
  if (
    bestFunnelRequest &&
    Game.rooms[bestFunnelRequest.roomName] &&
    Game.rooms[bestFunnelRequest.roomName].isMy &&
    Game.rooms[bestFunnelRequest.roomName].energyLevel < config.energyLevel.STOP_FUNNEL
  ) {
  ...
  if (neeedsHelp) {
    const rooms = [...Overlord.myRooms].sort(
      (a, b) => Game.map.getRoomLinearDistance(roomName, a.name) - Game.map.getRoomLinearDistance(roomName, b.name)
    );
    for (const room of rooms) {
      ...
      if (
        ...
        room.terminal.store[RESOURCE_ENERGY] < TERMINAL_ENERGY_THRESHOLD_TO_HELP
      ) {
        continue;
      }
...
StructureTerminal.prototype.manageMinerals = function () {
  ...
  const resourceRequestsSorted = resourceRequests
    ? resourceRequests.sort((a, b) => b.priority - a.priority)
    : undefined;
  ...
  for (const resourceType of resourceTypes) {
    ...
    if (storeAmount > mineralAmountToSend) {
      if (resourceRequestsSorted) {
        const priorityRequest = resourceRequestsSorted.find(
          (request) => request.resourceType === resourceType && request.terminal
        );
        if (priorityRequest) {
    ...
    if (
      this.gatherResource(resourceType, MINERAL_AMOUNT_TO_KEEP, {
        threshold: MINERAL_AMOUNT_TO_KEEP + amountNeeded,
      }) === OK
    ) {
      Business.cancelAllOrder(resourceType, roomName, ORDER_BUY);
      continue;
