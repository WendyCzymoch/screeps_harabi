...
PowerCreep.prototype.moveMy = function (target, range = 0) {
  ...
  if (data.creeps[this.name].stuck > 4 && Math.random() > 0.5) {
    ...
    const annoyingCreep = data.creeps[this.name].path[0]
      ? data.creeps[this.name].path[0].lookFor(LOOK_CREEPS)[0]
      : false;
    if (annoyingCreep) {
      this.say('🙏', true);
  ...
  if (
    (data.creeps[this.name].target && !data.creeps[this.name].target.isEqualTo(targetPos)) ||
    !data.creeps[this.name].path.length
  ) {
    const maxRooms = this.room.name === targetPos.roomName ? 1 : 16;
  ...
  if (data.creeps[this.name].path[0] && data.creeps[this.name].path[0].isEqualTo(this.pos)) {
    ...
    if (
      data.creeps[this.name].path[0] &&
      isEdgeCoord(data.creeps[this.name].path[0].x, data.creeps[this.name].path[0].y)
    ) {
      data.creeps[this.name].path.shift();
    }
  }
