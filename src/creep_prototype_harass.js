...
Creep.prototype.getHarassTargetRoomName = function () {
  ...
  const harassTargetRoomNames = Overlord.getHarassTargetRoomNames().filter(
    (roomName) => {
      return Game.map.getRoomLinearDistance(this.pos.roomName, roomName) <= 3;
    }
  );

  const harassTargetRoomName = getMinObject(
    harassTargetRoomNames,
    (roomName) => {
...
Creep.prototype.harassRoom = function (roomName) {
  Game.map.visual.line(this.pos, new RoomPosition(25, 25, roomName), {
    color: COLOR_NEON_RED,
    width: 2,
  });
  Game.map.visual.text("HARASS", new RoomPosition(25, 45, roomName), {
    color: COLOR_NEON_RED,
  });

  Game.harassing = Game.harassing || {};
  Game.harassing[this.id] = Game.harassing[this.id] || {};
  Game.harassing[this.id] = {
  ...
  const structuresToWreck = this.room
  ...
  const killerCreeps = hostileCreeps.filter((creep) =>
    creep.checkBodyParts(["attack", "ranged_attack", "heal"])
  );
  ...
  if (hostileStructure) {
    if (this.pos.getRangeTo(hostileStructure) > 3) {
      this.moveMy(
        { pos: hostileStructure.pos, range: 3 },
        { staySafe: false, ignoreMap: 1 }
      );
      return;
    }
