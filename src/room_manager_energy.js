...
Room.prototype.manageEnergy = function () {
  ...
  outer: for (const researcher of this.creeps.researcher) {
  ...
  if (!this.storage) {
    haulers.push(...this.getSupplyingRemoteHaulers())
  }

  outer: for (const creep of haulers) {
...
Room.prototype.fetchHub = function (distributor) {
  ...
  if (
    ...
    terminal.store[RESOURCE_ENERGY] >
      (level === 8 ? TERMINAL_ENERGY_THRESHOLD_MAX_RCL : TERMINAL_ENERGY_THRESHOLD_LOW_RCL) + TERMINAL_ENERGY_BUFFER
  ) {
...
Room.prototype.getHubEnergyRequestId = function () {
  ...
  const controllerLinkThreshold = 1 + this.laborer.numWork * 3

  if (
    controllerLink &&
    storageLink &&
    controllerLink.store.getUsedCapacity(RESOURCE_ENERGY) < controllerLinkThreshold
  ) {
  ...
  if (
    ...
    terminal.store[RESOURCE_ENERGY] <
      (this.controller.level > 7 ? TERMINAL_ENERGY_THRESHOLD_MAX_RCL : TERMINAL_ENERGY_THRESHOLD_LOW_RCL)
  ) {
    return terminal.id
  }
...
Room.prototype.manageEnergySupply = function (arrayOfCreeps) {
  const requests = this.getEnergyRequests(arrayOfCreeps.length || 1)
  ...
  while (true) {
    ...
    for (const request of freeRequests) {
      ...
      if (
        ...
        bestApplicant.pos.getRangeTo(existingRequest.pos) <= bestApplicant.pos.getRangeTo(request.pos)
      ) {
        continue
      }
...
Room.prototype.getEnergyRequests = function (number) {
  const requests = {}

  number = number || 1 // number should not be 0.

  let numRequests = 0

  if (numRequests >= number) {
    return requests
  }
  ...
  const energyCapacityUsed = this.energyCapacityAvailable - (this.energyAvailable || 0)

  if (energyCapacityUsed > 0) {
  ...
  if (
    ...
    factory.store.getUsedCapacity(RESOURCE_ENERGY) < 2000
  ) {
  ...
  if (nuker) {
    ...
    if (this.memory.fillNuker && nuker.store.getFreeCapacity(RESOURCE_ENERGY) > 0) {
      requests[nuker.id] = new Request(nuker)
    }
  }
...
Room.prototype.getEnergyDepots = function () {
  ...
  const droppedResources = this.find(FIND_DROPPED_RESOURCES).filter(
    (droppedResource) => droppedResource.resourceType === RESOURCE_ENERGY && droppedResource.amount > 100
  )
  const filteredDroppedResources = droppedResources.filter(
    (droppedResource) => droppedResource.pos.getClosestRange(this.sources) > 1
  )
...
Room.prototype.manageEnergyFetch = function (arrayOfCreeps) {
  ...
  while (true) {
    const freeRequests = requestsArray.filter(
      (request) => request.amount > (request.threshold || 0) && request.applicants.getSize()
    )
    if (!freeRequests.length) {
      break
    }
  ...
  for (const applicant of applicants) {
    ...
    if (creep.memory.role === 'hauler' && !this.heap.isEnemy) {
      ...
      if (source) {
        ...
        const waitingPos = creep.pos.findClosestByRange(
          source.waitingArea.filter((pos) => {
            return pos.walkable && creep.checkEmpty(pos) === OK
          })
        )
        if (waitingPos) {
