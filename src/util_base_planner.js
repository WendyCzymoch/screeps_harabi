...
Room.prototype.getBasePlanBySpawn = function () {
  const spawn = Util.getMinObject(this.structures.spawn, (spawn) => spawn.pos.x)
  if (!spawn) {
...
Room.prototype.getBasePlanByRegion = function (region) {
  ...
  candidates.sort((a, b) => a.getRangeTo(this.controller) - b.getRangeTo(this.controller))

  for (const candidate of candidates) {
    if (candidate.getRangeTo(this.controller) < 5) {
      continue
    }
    if (candidate.getClusterAnchor(mincut.costsForBasePlan)) {
      if (CANDIDATE_VISUAL) {
...
Room.prototype.getRegions = function () {
  ...
  const items = [DT, exitFF, sourceFF, controllerFF]
  ...
  outer: for (const level of levels) {
    ...
    for (const pos of positions[level]) {
      ...
      getRegion: while (levelNow > 0) {
        ...
        let isRegion = true
        floodFill: while (queue.length > 0) {
...
Room.prototype.getBasePlanAfterMincut = function (pos, inputCosts, mincut, costsForMincut) {
  ...
  const possiblePositions = this.controller.pos
    .getInRange(2)
    .filter((pos) => !pos.isWall)
    .sort((a, b) => a.getClosestPathLength([firstAnchor.pos]) - b.getClosestPathLength([firstAnchor.pos]))

  let controllerLinkPos = undefined
  for (const posClosestToController of possiblePositions) {
    // use floodfill to expand
    const nearClosestPositions = this.floodFill([posClosestToController], {
      maxLevel: 8,
      costMatrix: costsForRoad,
    }).allPositions
    nearClosestPositions.push(posClosestToController)

    // filter by range to controller<=2 and sort by num of available spots
    let controllerLinkCandidates = nearClosestPositions.filter(
      (pos) =>
        pos.getRangeTo(this.controller.pos) <= 2 &&
        !pos.isWall &&
        costs.get(pos.x, pos.y) !== ROAD_COST &&
  ...
  let isLab = false
  outerLab: for (const firstSourceLab of sourceLabPositions) {
  ...
  for (const source of sources) {
    const sourcePathSearch = PathFinder.search(
      firstSpawnPos,
      { pos: containerPositions[source.id], range: 1 },
      {
        plainCost: 2,
        swampCost: 4,
        roomCallback: function (roomName) {
  ...
  const mineralPathSearch = PathFinder.search(
    firstSpawnPos,
    { pos: mineralContainerPos, range: 1 },
    {
      plainCost: 2,
      swampCost: 4,
      roomCallback: function (roomName) {
  ...
  const rampartEfficiency = Math.max(
    0,
    Math.min(1, (RAMPART_NUM_MAX - numRamparts) / (RAMPART_NUM_MAX - RAMPART_NUM_MIN))
  )
  ...
  for (const source of this.sources) {
    inner: for (const adjacent of source.pos.getAtRange(1)) {
      if (adjacent.isWall) {
        continue
      }
      if (inputCosts.get(adjacent.x, adjacent.y) < OUTSIDE_COST) {
...
RoomPosition.prototype.calcTowerDamage = function (target) {
...
Room.prototype.getBasePlanByChunk = function () {
  ...
  const bestPos = Util.getMinObject(candidates, (pos) => {
...
Room.prototype.getBestChunk = function () {
  ...
  const bestLabel = Util.getMaxObject(labels, (label) => {
    if (label === 1) {
      return -Infinity
    }
    return scores[label]
