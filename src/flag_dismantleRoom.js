Flag.prototype.dismantleRoom = function () {
  ...
  if (this.memory.lastStructure) {
    const lastStructurePos = new RoomPosition(
...
Creep.prototype.dismantleToController = function (roomName) {
  if (
    !(
      this.heap.dismantleToController &&
      this.heap.dismantleToController.targetPos &&
      this.heap.dismantleToController.path.length
    )
  ) {
