...
Room.prototype.sendBlinkies = function (request) {
  ...
  const ticksToLiveMin = Math.min(
    Math.min(...blinkies.map((creep) => creep.ticksToLive || 1500)),
    1500
  );
  ...
  if (request.status === "produce") {
    ...
    if (
      blinkies.some((creep) => creep.spawning || creep.memory.boosted === false)
    ) {
      return;
    }
...
function runBlinky(creep, targetRoomName) {
  ...
  const enemyCombatants = creep.room
    ...
    .filter(
      (creep) =>
        !creep.pos.isRampart && creep.owner.username !== "Source Keeper"
    );
  ...
  if (
    creep.heap.enemyLastDetectionTick !== undefined &&
    Game.time < creep.heap.enemyLastDetectionTick + 2
  ) {
    return;
  }
  ...
  const hostileCreeps = creep.room
    .findHostileCreeps()
    .filter((creep) => !creep.pos.isRampart);
  ...
  if (
    creep.heap.enemyLastDetectionTick !== undefined &&
    Game.time < creep.heap.enemyLastDetectionTick + 5
  ) {
    return;
  }
  ...
  if (creep.room.isMy) {
    if (creep.pos.getRangeTo(creep.room.controller.pos) > 5) {
      creep.moveMy(
        { pos: creep.room.controller.pos, range: 5 },
        { staySafe: false }
      );
    }
