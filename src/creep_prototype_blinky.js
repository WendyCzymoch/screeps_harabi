...
Creep.prototype.handleCombatants = function (targets) {
    ...
    if (range > idealRange) {
        this.moveMy(
            { pos: closestTarget.pos, range: idealRange },
            { ignoreCreeps: false, staySafe: false, ignoreMap: 2 }
        );
        return;
    }
...
Creep.prototype.harasserRangedAttack = function (attackNeutralStructures = false) {
    ...
    for (const pos of positions) {
        ...
        if (rangedAttackTarget === undefined) {
            ...
            priorityTarget.hits / priorityTarget.hitsMax <
            rangedAttackTarget.hits / rangedAttackTarget.hitsMax
        ) {
            rangedAttackTarget = priorityTarget;
        } else if (
            priorityTarget.hits / priorityTarget.hitsMax === rangedAttackTarget.hits / rangedAttackTarget.hitsMax &&
            priorityTarget.healPower > rangedAttackTarget.healPower
        ) {
            rangedAttackTarget = priorityTarget;
        }
