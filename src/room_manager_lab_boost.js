...
Room.prototype.manageBoost = function (requests) {
  ...
  const targetRequest = Util.getMinObject(requests, (request) => request.time)
...
Room.prototype.prepareBoostResources = function () {
  ...
  for (const resourceType of resourceTypes) {
    ...
    if (
      targetLab.mineralType &&
      targetLab.mineralType !== resourceType &&
      targetLab.store.getUsedCapacity(targetLab.mineralType) > 0
    ) {
...
Room.prototype.operateBoost = function (boostRequest) {
  ...
  const reactionLabs = this.structures.lab
  ...
  if (this.memory.boostState === 'boost') {
    ...
    for (let i = 0; i < resourceTypes.length; i++) {
      ...
      const lab = this.structures.lab.find(
        (lab) => lab.store[resourceType] >= requiredResources[resourceType].mineralAmount
      )
