...
Room.prototype.manageRemotes = function () {
  ...
  const invaderStrengthThreshold = getInvaderStrengthThreshold(
  ...
  outer: for (const info of activeRemotes) {
    ...
    if (targetRoom) {
      const invaders = [...targetRoom.findHostileCreeps()].filter(
        (creep) => creep.owner.username !== "Source Keeper"
      );
      const enemyInfo = getCombatInfo(invaders);
      const isEnemy = invaders.some((creep) =>
        creep.checkBodyParts(INVADER_BODY_PARTS)
      );
      const invaderCore = targetRoom
        .find(FIND_HOSTILE_STRUCTURES)
        .find(
          (structure) => structure.structureType === STRUCTURE_INVADER_CORE
        );
      ...
      if (!memory.isCombatant && enemyInfo.strength > 0) {
        const maxTicksToLive = Math.max(
          ...invaders.map((creep) => creep.ticksToLive)
        );
    ...
    if (memory.isCombatant) {
      ...
      Game.map.visual.text(
        `👿${leftTicks}`,
        new RoomPosition(49, 5, targetRoomName),
        {
          fontSize: 5,
          align: "right",
        }
    ...
    if (
      ...
      (!status.constructionComplete ||
        Game.time > status.constructionCompleteTime + 3000)
    ) {
...
Room.prototype.manageRemoteHaulers = function () {
  ...
  const haulers = Overlord.getCreepsByRole(this.name, "remoteHauler").filter(
    (creep) => {
      if (creep.spawning) {
        return false;
      }
  ...
  for (const hauler of haulers) {
    if (hauler.memory.targetRoomName) {
      ...
      if (
        !hauler.memory.supplying &&
        hauler.memory.sourceId &&
        sourceStats[hauler.memory.sourceId]
      ) {
        sourceStats[hauler.memory.sourceId].energyAmountNear -=
          hauler.store.getCapacity();
      }
    }
    if (!hauler.memory.targetRoomName) {
  ...
  for (const hauler of freeHaulers) {
    ...
    if (score > 0) {
      ...
      hauler.say("😴", true);
      this.heap.numIdlingRemoteHaulerCarryParts +=
        hauler.getActiveBodyparts(CARRY);
...
function runRemoteHauler(creep, base, targetRoomName) {
  ...
  if (roomType === "sourceKeeper") {
    if (
      creep.pos.findInRange(hostileCreeps, SOURCE_KEEPER_RANGE_TO_START_FLEE)
        .length > 0
    ) {
    ...
    if (
      creep.pos.findInRange(keeperLairs, KEEPER_LAIR_RANGE_TO_START_FLEE)
        .length > 0
    ) {
  ...
  const container = source.pos
    ...
    .find(
      (structure) =>
        structure.store && structure.store.getUsedCapacity() >= energyThreshold
    );
...
Room.prototype.constructRemote = function (
  targetRoomName,
  ...
  if (!targetRoomName && constructionComplete) {
    ...
    for (const remoteBuilder of remoteBuilders) {
      if (
        remoteBuilder.room.name !== this.name ||
        isEdgeCoord(remoteBuilder.pos.x, remoteBuilder.pos.y)
      ) {
  ...
  const remoteInfoToConstruct = activeRemotes.find(
    (info) => info.remoteName === roomNameToConstruct
  );

  const remoteInfo = activeRemotes.find(
    (info) => info.remoteName === targetRoomName
  );
  ...
  if (remoteInfoToConstruct) {
    ...
    const resourceIdsToConstruct = [
      ...remoteInfoToConstruct.resourceIds,
    ].filter((id) => !constructBlueprints[id].isMineral);

    const remoteBuilders = Overlord.getCreepsByRole(
      this.name,
      "remoteBuilder"
    ).filter((creep) => {
      if (creep.spawning) {
  ...
  remoteStatus.constructionComplete =
    remoteStatus.constructionComplete || false;
  ...
  if (creep.memory.working) {
    ...
    if (
      ...
      if (!creep.heap.targetId || !Game.getObjectById(creep.heap.targetId)) {
        creep.heap.targetId =
          creep.pos.findClosestByRange(constructionSites).id;
      }
...
Room.prototype.spawnRemoteWorkers = function (
  remotesToSpawn,
  constructionComplete
) {
  const fontSize = 4;
  ...
  Game.map.visual.text(
    `🚚${numCarry}/${maxCarry} `,
    new RoomPosition(25, 35, this.name),
    {
      fontSize,
  ...
  if (this.heap.numIdlingRemoteHaulerCarryParts > 0) {
    const idlingPercentage =
      Math.floor(
        (this.heap.numIdlingRemoteHaulerCarryParts / numCarry) * 10000
      ) / 100;
    Game.map.visual.text(
      `😴${idlingPercentage}%`,
      new RoomPosition(25, 40, this.name),
      {
        fontSize,
...
Room.prototype.getNeedBigMiner = function () {
  ...
  return (this._needBigMiner =
    ...
    (avgCpu / Game.cpu.limit > 0.8 || Game.cpu.bucket < 9000));
};
...
Room.prototype.spawnSourceKeeperRemoteWorkers = function (info, options) {
  ...
  for (const resourceId of resourceIds) {
    ...
    if (
      this.heap.remoteNameToConstruct &&
      this.heap.remoteNameToConstruct === targetRoomName
    ) {
  ...
  const miners = Overlord.getCreepsByRole(targetRoomName, "remoteMiner").filter(
    (creep) => {
      if (creep.spawning) {
        return true;
      }
      ...
      return (
        creep.ticksToLive > creep.body.length * CREEP_SPAWN_TIME + pathLength
      );
    }
  );
  ...
  for (const sourceId of Object.keys(sourceStat)) {
    ...
    const black = "#000000";
    Game.map.visual.text(
      `⛏️${stat.work}/${stat.maxWork}`,
      new RoomPosition(x, y, targetRoomName),
      {
        fontSize,
    ...
    if (source && source instanceof Source) {
      ...
      Game.map.visual.text(
        `🔋${amountNear}/2000 `,
        new RoomPosition(x, y + 5, targetRoomName),
        {
          fontSize,
  ...
  if (
    !sourceKeeperHandlers.find(
      (creep) => creep.ticksToLive > 200 || creep.spawning
    )
  ) {
...
Room.prototype.requestSourceKeeperHandler = function (
  targetRoomName,
  resourceIds
) {
  if (!this.hasAvailableSpawn()) {
...
Room.prototype.spawnNormalRemoteWorkers = function (info, options) {
  ...
  Game.map.visual.text(
    `⏰${reservationTick}`,
    new RoomPosition(49, 45, targetRoomName),
    {
      fontSize: 5,
      align: "right",
    }
  ...
  if (reservationTick < 0) {
    if (canReserve) {
      ...
      const numClaimParts = reservers
        .map((creep) => creep.getActiveBodyparts("claim"))
        .reduce((a, b) => a + b, 0);

      if (
        numClaimParts < 2 &&
        reservers.length < (remoteStatus.controllerAvailable || 2)
      ) {
  ...
  if (
    ...
    for (const resourceId of resourceIds) {
      ...
      const haulerRatio = Math.clamp(
        (sourceStat[resourceId].maxCarry - deficiency) /
          sourceStat[resourceId].maxCarry,
        0,
        1
      );
  ...
  for (const resourceId of resourceIds) {
    ...
    Game.map.visual.text(
      `⛏️${stat.work}/${maxWork}`,
      new RoomPosition(x, 20, targetRoomName),
      {
        fontSize,
    ...
    if (source) {
      ...
      Game.map.visual.text(
        `🔋${amountNear}/2000 `,
        new RoomPosition(x, 25, targetRoomName),
        {
          fontSize,
          backgroundColor: amountNear < 2000 ? black : COLOR_NEON_RED,
          opacity,
  ...
  if (memory.invaderCore) {
    const coreAttackers = Overlord.getCreepsByRole(
      targetRoomName,
      "coreAttacker"
    );
  ...
  if (canReserve && reservationTick < RESERVATION_TICK_THRESHOLD) {
    ...
    if (
      numClaimParts < 2 &&
      reservers.length < (status.controllerAvailable || 2)
    ) {
      this.requestReserver(targetRoomName);
      result.requested = true;
      return result;
...
Room.prototype.requestRemoteMiner = function (
  targetRoomName,
  sourceId,
  options = {}
) {
...
function getRemoteMinerModel(energyCapacity, maxWork) {
  ...
  while (cost < energyCapacity && work + move + carry < MAX_CREEP_SIZE) {
    if (
      (move === 0 || work / move >= 2) &&
      energyCapacity >= cost + BODYPART_COST[MOVE]
    ) {
      move++;
    ...
    if (
      work >= 5 &&
      carry < 1 &&
      energyCapacity >= cost + BODYPART_COST[CARRY]
    ) {
      carry++;
  ...
  if (interval >= CREEP_LIFE_TIME) {
    if (remoteStatus.netIncomePerTick) {
      ...
      remoteStatus.netIncomePerTick =
        modifiedAlpha * (netIncome / interval) +
        (1 - modifiedAlpha) * remoteStatus.netIncomePerTick;
    } else {
...
Room.prototype.getActiveRemotes = function () {
  if (
    ...
    this.memory.activeRemotesTime &&
    Game.time < this.memory.activeRemotesTime + 20
  ) {
  ...
  for (const remoteName of this.getRemoteNames()) {
    ...
    if (
      remoteStatus.roomType === "sourceKeeper" &&
      this.energyCapacityAvailable < 4270
    ) {
    ...
    const betterSourceValue =
      value.resources[betterSourceId] + (value.reserve || 0);
    const betterSourceWeight =
      spawnUsage.resources[betterSourceId] + (spawnUsage.reserve || 0);
    const info2 = {
      remoteName,
      intermediates,
  ...
  let spawnCapacityForRemotes = Math.floor(
    this.structures.spawn.length * 485 - this.getBasicSpawnCapacity()
  );
  ...
  for (let i = 0; i < remoteInfos.length; i++) {
    ...
    for (let j = spawnCapacityForRemotes; j > 0; j--) {
      ...
      if (
        intermediateNames &&
        intermediateNames.some(
          (intermediateName) => !resultRemoteNames.includes(intermediateName)
        )
      ) {
        continue;
      }
...
Room.prototype.getNormalRemoteValue = function (targetRoomName) {
  ...
  for (const blueprint of Object.values(blueprints)) {
    ...
    const minerCost =
      (needBigMiner ? 1600 : 950) / (CREEP_LIFE_TIME - distance);
    const haluerCost =
      (blueprint.maxCarry * (canReserve ? 75 : 100)) / CREEP_LIFE_TIME;
  ...
  if (canReserve) {
    result.reserve =
      (-1 * (BODYPART_COST[CLAIM] + BODYPART_COST[MOVE])) /
...
function isStronghold(targetRoomName) {
  ...
  if (!targetRoom) {
    if (invaderCoreInfo) {
      if (
        invaderCoreInfo.deployTime &&
        Game.time < invaderCoreInfo.deployTime
      ) {
        return false;
      }

      if (
        invaderCoreInfo.ticksToCollapse &&
        Game.time < invaderCoreInfo.ticksToCollapse
      ) {
        Game.map.visual.text(
          invaderCoreInfo.ticksToCollapse - Game.time,
          new RoomPosition(40, 5, targetRoomName),
          { fontSize: 6 }
        );
        return true;
      }
  ...
  for (const resource of resources) {
    ...
    const search = PathFinder.search(
      ...
      { pos: startingPoint, range: 1 },
      {
        ...
        roomCallback: function (roomName) {
          ...
          const currentRoomResources = [
            ...currentRoom.find(FIND_SOURCES),
            ...currentRoom.find(FIND_MINERALS),
          ];
    ...
    for (const pos of path) {
      ...
      if (
        ...
        remoteStatus &&
        !isMineral &&
        pos.findInRange(dangerSpots, 5).length > 0
      ) {
...
Room.prototype.getStoragePos = function () {
  ...
  const storagePlan = lv4.find(
    (plan) => plan.structureType === STRUCTURE_STORAGE
  );
  if (storagePlan) {
...
function runAway(creep, roomName) {
  ...
  if (creep.memory.keeperLairId) {
    ...
    if (
      keeperLair &&
      keeperLair.ticksToSpawn < 15 &&
      creep.pos.getRangeTo(keeperLair.pos) < 8
    ) {
