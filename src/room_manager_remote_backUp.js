...
Room.prototype.getActiveRemotes = function () {
    ...
    for (const remoteName of this.getRemoteNames()) {
        ...
        const info = { remoteName, intermediate, value, weight, oneSource: false, block }
        ...
        const info2 = { remoteName, intermediate, value: value2, weight: weight2, oneSource: true, block }
...
function spawnRemoteWorkers(room, targetRoomNames) {
    ...
    const needBigMiner = room.controller.level >= 6 && (avgCpu / (Game.cpu.limit) > 0.8 || Game.cpu.bucket < 9000)
    ...
    for (const targetRoomName of targetRoomNames) {
        ...
        for (const info of blueprint) {
            ...
            if (stat.work < maxWork && stat.numMiner < info.available) {
                ...
                room.requestRemoteMiner(targetRoomName, sourceId, { containerId, maxWork: numWork, needCarry: canReserve })
                requested = true
                continue outer
            }
            ...
            const ratio = room.storage ? 1 : 0.8
            const energyCapacity = Math.max(room.energyCapacityAvailable * ratio, 300)
