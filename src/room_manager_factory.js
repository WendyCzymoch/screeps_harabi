Room.prototype.factoryDistribution = function () {
    ...
    if (terminal.store['purifier'] > 0) {
        if (!researcher) {
            this.heap.needResearcher = true
            return
        }
        return researcher.getDeliveryRequest(terminal, factory, 'purifier')
    }

    if (factory.store['purifier'] > 0) {
        factory.produce('X')
    }

    if (factory.store['X'] > 0) {
        if (!researcher) {
            this.heap.needResearcher = true
            return
        }
        return researcher.getDeliveryRequest(factory, terminal, 'X')
    }
}
