Object.defineProperties(Source.prototype, {
  waitingArea: {
    get() {
      ...
      return (this._waitingArea = this.heap.waitingArea =
        [...floodFill[2], ...floodFill[3]].filter((pos) => costs.get(pos.x, pos.y) < 5));
    },
  },
  available: {
    get() {
  ...
  info: {
    get() {
      ...
      this._info = {};
      const miners = this.room.creeps.miner.filter(
        (creep) =>
          (creep.ticksToLive || 1500) > this.range.spawn + 3 * creep.body.length && creep.memory.sourceId === this.id
      );
      const haulers = this.room.creeps.hauler.filter(
        (creep) => (creep.ticksToLive || 1500) > 3 * creep.body.length && creep.memory.sourceId === this.id
      );
      ...
      this._info.maxCarry = this.linked
        ? 0
        : this.room.controller.linked || this.room.storage || this.room.constructionSites.length > 0
          ? Math.ceil(0.4 * this.range.spawn) + 2
          : Math.ceil(0.4 * this.range.controller) + 2;

      this._info.maxNumHauler = Math.ceil(
        this._info.maxCarry / (Math.floor(this.room.energyCapacityAvailable / 150) * 2)
      );
  ...
  droppedEnergies: {
    get() {
      if (!this._droppedEnergies) {
        this._droppedEnergies = this.pos
