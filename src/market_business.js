...
Business.getMaxBuyOrder = function (resourceType, roomName, log) {
  ...
  const buyOrders = Game.market
    .getAllOrders({ resourceType: resourceType, type: ORDER_BUY })
    .filter((order) => !myOrdersId.includes(order.id))
  const maxBuyOrder = Util.getMaxObject(buyOrders, (order) => {
...
Business.getMinSellOrder = function (resourceType, roomName) {
  ...
  const sellOrders = Game.market
    .getAllOrders({ resourceType: resourceType, type: ORDER_SELL })
    .filter((order) => !myOrdersId.includes(order.id))
  const minSellOrder = Util.getMinObject(sellOrders, (order) => {
...
Business.buy = function (resourceType, amount, roomName) {
  ...
  const myOrders = this.myOrders
  const targetOrders = myOrders.filter((order) => {
    if (roomName && order.roomName !== roomName) {
      return false
    }
    if (order.resourceType !== resourceType) {
      return false
  ...
  if (resourceType === 'pixel') {
    console.log('current:' + JSON.stringify(currentOrder))
  }
  ...
  if (!currentOrder) {
    time = 0
  } else if (currentOrder.created) {
    time = Game.time - currentOrder.created
  } else if (currentOrder.createdTimestamp) {
    time = (new Date().getTime() - currentOrder.createdTimestamp) / 1000 / 4
  }

  const priceNow =
    startPrice + (priceRange.max - startPrice) * Math.floor(time / TICKS_TO_CHANGE_PRICE) * CHANGE_PRICE_RATIO

  if (resourceType === 'pixel') {
    console.log('priceNow:' + JSON.stringify(priceNow))
  }
  ...
  if (resourceType === 'pixel') {
    console.log('minSellOrder:' + JSON.stringify(minSellOrder))
  }
...
Business.sell = function (resourceType, amount, roomName = undefined) {
  ...
  const priceNow =
    priceRange.max -
    (priceRange.max - priceRange.avgPrice) * Math.floor(time / TICKS_TO_CHANGE_PRICE) * CHANGE_PRICE_RATIO
