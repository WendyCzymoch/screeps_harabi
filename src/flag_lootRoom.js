...
Creep.prototype.lootRoom = function () {
  if (this.memory.supplying && this.store.getUsedCapacity() === 0) {
    ...
    (this.ticksToLive < 50 * (this.memory.routeLength || 1) || this.store.getFreeCapacity(RESOURCE_ENERGY) === 0)
  ) {
  ...
  if (this.memory.supplying) {
    ...
    if (this.room.name !== this.memory.base) {
      this.moveMy({ pos: storage.pos, range: 1 })
      return
    }
  ...
  if (
    targetCached &&
    targetCached.store &&
    (targetCached.store.getUsedCapacity() || targetCached.store.getUsedCapacity(RESOURCE_ENERGY))
  ) {
  ...
  const remainStructures = targetRoom
    .find(FIND_STRUCTURES)
    .filter(
      (structure) =>
        structure.store &&
        (structure.store.getUsedCapacity() >= 300 || structure.store.getUsedCapacity(RESOURCE_ENERGY) >= 100)
    )
